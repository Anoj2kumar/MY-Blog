...
const options = ["Travel", "Company", "College", "Fitness", "Sports", "Food", "Tech", "University"];
const AddEditBlog = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if(title && description && imageUrl && category)
        {
            ...
            if(!editMode){
                ...
                const response = await axios.post("http://localhost:3001/blog",updatedBlogData);
                ...
                const response = await axios.put(`http://localhost:3001/blog/${id}`,formValue);
    ...
    const onUploadImage = (file) => {
        ...
        formData.append("upload_preset", "emcd5igl");
        axios
        .post("http://api.cloudinary.com/v1_1/dzvx2y6hm/image/upload", formData)
        .then((resp) => {
    ...
    return (
        <MDBValidation className="row g-3" style={{marginTop: "100px"}} noValidate onSubmit={handleSubmit}>
          ...
          <div style={{
                  ...
                  {categoryErrMsg && (
                      <div className="categoryErrorMsg">Please select a category</div>
                  )}
                  <br />
                  <br />
